  ...
  FutData(n)
{
  ...
  // variable is also used in the localization_node.
  dt_amostr = DT_AMOSTR_INICIAL;
...
void Control::set_bypass_control(bool new_bypass_control[3], const PWM_ROBOTS& new_pwm)
{
  for(int i=0; i < 3; i++)
    bypassControl[i] = new_bypass_control[i];

  pwm = new_pwm;
}

void Control::set_bloqueado(bool new_bloqueado[3])
{
  for(int i=0; i < 3; i++)
    bloqueado[i] = new_bloqueado[i];
}
