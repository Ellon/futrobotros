...
/**  E' uma implementacao de campo potencial (paradigma de planejamento de trajetoria) 
   de forma discretizada (nao e' vetorial),  o campo e' dividido em uma matriz 9x10.
   A classe e' usada  para definir o posicionamento do jogador "sem bola" de forma 
   que se afaste das paredes, da area de defesa, de todos outros robos e tambem da 
   bola para estar sempre livre assumindo o papel do jogador "com bola", caso este 
   fique preso. Criada em 2010 **/     
class Repulsao {
private:
  unsigned cell[9][10];
...
/** Define regioes que sempre serão de repulsão: paredes, 
    laterais, paredes de fundo (de ataque e defesa) e a
    regiao da area de defesa **/ 
...
/** Metodo para incluir obstaculos moveis na grade de campo potencial
    a partir da posicao do objeto, a celula correspondente tem repulsao 3
    todas celulas adjacentes da celula (direita, esquerda, acima e abaixo)
    onde o objeto esta localizado recebem repulsao 2 e
    a celulas nas diagonais (cima esquerda, baixo esquerda, direita cima e
    direita baixo) recebem repulsao 1
*/ 
void Repulsao::incluirObstaculo(POS_BOLA obs)
{
  ...
  // if (obs.y() >= FIELD_HEIGHT/2.0) obs.x() = 0.99*FIELD_HEIGHT/2.0;
  if (obs.y() <= -FIELD_HEIGHT/2.0) obs.y() = -0.99*FIELD_HEIGHT/2.0;
  if (obs.y() >= FIELD_HEIGHT/2.0) obs.y() = 0.99*FIELD_HEIGHT/2.0;
...
/** Conversao de posicao da grade para posicionamento em metros
    e calculado o centro daquala posica da grade 
 **/
...
/** Este metodo retorna qual a melhor posicao em metros (coord. de mundo)
    da celula que corresponde a posicao passada por parametro  
**/
...
/**A  Classe Strategy define como os jogadores devem se movimentar para alcancar conseguir seus objetivos,
   possui 3 niveis de hierarquia, para organizacao:
   1 definicao de funcoes:
     cada jogador pode ser ou "goleiro" ou "sem_bola" ou "com_bola"
   2 definicao de acoes:
     com papel definido, cada jogador  tera sua ação calculada.
     Cada papel tem suas acoes especificas
   3 execucao das acoes:
     Cada acao e' uma forma de definir a posicao desejada (referencia) para
     o robo.
   
   Este niveis sao implementados em diferentes metodos da classe Stragtegy
  
   A estrategia de uma forma geral foi desenvolvida observando as 
   regras da categoria IEEE Very Small Size e por TESTES REAIS.
  
**/
Strategy::Strategy(ros::NodeHandle& n)
: FutData(n)
{
  // Initialization that was in FutData  
  ...
  // Workaround to handle id_pos. This was a FutData attribute, that was
  // incremented by FutRobot, and used in acquisition and strategy. Here we
  // set it to zero.
  id_pos = 0;
...
/** Principal metodo da classe, ele esta dentro do laco do sistema como todo. 
   chama todos os metodos necessarios para cada etapa da estrategia.
   -Analise da posicao da bola, dos jogadores
   -Decisao da funcao de cada jogador do time 
   -Decisao do acao de acordo com a situacao
   -Execucao das acoes. **/
bool Strategy::strategy()
{
  ...
  // Workaround to handle id_pos. This was a FutData attribute, that was
  // incremented by FutRobot, and used in acquisition and strategy. Here we
  // just incement it, because it may be needed later below.
  id_pos++;
  ...
  //  cout << "analiza jogador\n"; cout.flush();
  analisa_jogadores();
  // analisa posiçao dos adversários
  //  cout << "escolher adversarios\n"; cout.flush();
  analisa_adversarios();
  /*
    cout << bola_dentro_gol << "  " <<  bola_area_defesa  << "  " 
    << bola_frente_area << "  " << bola_lado_area << "  " 
    << bola_parede_fundo << "  " <<  bola_quina_fundo << "  "
    << bola_fundo << "  " <<  bola_parede_lateral << "  " 
    << bola_lateral << "  " << bola_quina_frente << "  "
    << bola_parede_frente << "  " << bola_regiao_central << "  "  << bola_frente << endl;
  */
  // detecta se algum dos robos esta bloqueado


  //  cout << "deteccao de bloqueados \n"; cout.flush();
  detecta_bloqueados();

  // escolhe uma funcao (goleiro, com_bola, sem_bola) para cada jogador
  //  cout << "definicao de funcoes \n"; cout.flush();
  escolhe_funcoes();

  
  //  cout << "escolher acao de jogador\n"; cout.flush();
  ...
  //  cout << "calculo de referencias\n"; cout.flush();
  // calcula as referencias a partir das acoes
  for (i=0; i<3; i++) {
    //    cout << papeis.me[i].acao << endl; cout.flush();
  ...
  //  cout << "fim de estrategia\n"; cout.flush();
  return false;
}



/**  Este metodo serve para testar as condições de cada jogador do nosso time
     de acordo com os predicados (VARIAVEIS BOOLEANAS) da estrategia cada 
     funcao possui seus proprios predicados. O "goleiro" algumas, o "com bola"
     varios e o "sem bola" alguns.   
     os predicados sao definidos de forma geometrica.
...
/** Aqui sao definidos os predicados relativos aos jogadores
    adversarios.  
**/
...
/** Neste metodo sao analizadas as condicoes da bola acerca de velocidade 
   e da posicao no campo. Para evitar condicoes chaveamento no jogo, a maioria
   dos predicados incorpora uma histerese **/ 
...
/** Testa se os nossos jogadores estam ou nao bloqueados,
    Consideramos um jogador como bloqueado quando nao responde aos
    comandos a ele enviados, durante um certo tempo.
**/
...
/** Neste Metodo e' definido qual jogador sera "goleiro",
    "com bola" e o "sem bola"
 **/
...
void Strategy::escolhe_funcoes()
{
  ...
  case CALIBRATION_CONTROL_STATE:
...
/** Este metodo de acordo com a situacao do jogo 
    (falta, penalti...) vai definir o papel do goleiro
    no jogo corrido chama o metodo acao_goleiro_play
**/
void Strategy::acao_goleiro(int id)
{
  ...
  switch(gameState()){
  case CALIBRATION_CONTROL_STATE:
        papeis.me[id].acao = CALIBRATION_CONTROL_CIRCLE;
	//        papeis.me[id].acao = CALIBRATION_CONTROL_LINEAR_POSITION_1;
    break;
  case PAUSE_STATE:
  case FINISH_STATE:
    papeis.me[id].acao = ESTACIONAR;
...
/** Quando o jogo esta em andamento  e' este metodo
    que e chamado para definir qual a acao do goleiro
 **/
...
/** Define qual a acao do jogador "com bola" 
   em situacoes gerais.
**/ 
...
/** Define a acao do jogar "com bola" durante o 
    jogo corrido.
 **/
...
/**
    Define qual a acao do jogador "sem bola".
    em qualquer situacao de jogo, seja em andamento, em penalti...
 **/
...
/**
    Este e' metodo  de nivel mais baixo na estrategia,
    onde sao executadas as acoes das tres funcoes.
    Cada acao e' nomeado com um prefixo que identifica 
    se ela e' especifica do
         "goleiro"  ->  G_XXXXX  
         "com bola" ->  A_XXXXX
         "sem bola" ->  D_XXXXX
    acoes sem estes prefixos, sao gerais  
    qualquer um dos jogadores pode fazer.
**/
void Strategy::calcula_referencias(int id)
{
  ...
  case CALIBRATION_CONTROL_LINEAR_POSITION_1:
  ...
  case CALIBRATION_CONTROL_LINEAR_POSITION_2:
    ref.me[0].x() = -0.50;
    ref.me[0].y() = 0.40;
    ref.me[0].theta() = 0.00;
  ...
  case CALIBRATION_CONTROL_ANGULAR_POSITION_1:
  ...
  case CALIBRATION_CONTROL_ANGULAR_POSITION_2:
    ref.me[0].x() = -0.60;
    ref.me[0].y() = 0.49;
    ref.me[0].theta() = 0.00;
    ...
    //***************************************Test 3: Circuling the Center*****************************************************

  case CALIBRATION_CONTROL_CIRCLE:
    
    {    
      double tempo_volta0 = 5.0;  // Uma volta em 5 segundos
      double ang_circulo0 = ang_equiv(2.0*M_PI*id_pos/(30.0*tempo_volta0));
      
      ang_circulo0 += id*2.0*M_PI/3.0;
      ref.me[0].x() = 1.0*CIRCLE_RADIUS*cos(ang_circulo0);
      ref.me[0].y() = 1.0*CIRCLE_RADIUS*sin(ang_circulo0);
      ref.me[0].theta() = ang_circulo0+M_PI_2;
      
      double tempo_volta1 = 5.0;  // Uma volta em 5 segundos
      double ang_circulo1 = ang_equiv(2.0*M_PI*id_pos/(30.0*tempo_volta1));
      ang_circulo1 += id*2.0*M_PI/3.0;
      ref.me[1].x() = 1.5*CIRCLE_RADIUS*cos(ang_circulo1);
      ref.me[1].y() = 1.5*CIRCLE_RADIUS*sin(ang_circulo1);
      ref.me[1].theta() = ang_circulo1+M_PI_2;
      
      double tempo_volta2 = 5.0;  // Uma volta em 5 segundos
      double ang_circulo2 = ang_equiv(2.0*M_PI*id_pos/(30.0*tempo_volta2));
      ang_circulo2 += id*2.0*M_PI/3.0;
      ref.me[2].x() = 2.0*CIRCLE_RADIUS*cos(ang_circulo2);
      ref.me[2].y() = 2.0*CIRCLE_RADIUS*sin(ang_circulo2);
      ref.me[2].theta() = ang_circulo2+M_PI_2;
    }
...
/**
    Metodo para posicionamento do robo de frente para 
    a parede, com intuito de descolar a bola, chama outros 
    tres metodos da classe para realizar a tarefa.
    Retorna a posicao de referecia (em coordenadas de mundo) 
 **/
...
/** Este Metodo Calcula um sistema de coordenadas de acordo
    com a regiao da parede onde a bola esta encostada 
    alem de translacao pode ocorrer rotacao caso a bola
    esteja numa das quatros quinas do campo
 **/
...
/** Define uma conversao do sistema de coordenadas do mundo
   (centro do campo)  para o sistema de coordenadas relativo 
   a parede
 **/
static POS_BOLA posicao_relativa(POS_BOLA pos, POS_ROBO origem)
{
...
/** Conversao de coordenadas relativa a parede para o sist.
    de coordenadas do mundo (centro do campo).
 **/
static POS_ROBO posicao_absoluta(POS_ROBO pos, POS_ROBO origem)
{
...
/**
   Executa a acao de descolar a bola da parede, 
   o movimento e' uma curva  
**/
